...
class Student extends DB{
    ...
    protected function editStudent($id, $fname, $lname, $email,  $address, $phone, $course, $year){
        $connection = $this->dbOpen();
        $stmt = $connection->prepare("UPDATE students SET first_name = ?, last_name = ?, email = ?, address = ?, parents_contact =?, student_course = ?, student_year = ? WHERE id = ?");
        $stmt->execute([$fname, $lname, $email,  $address, $phone, $course, $year, $id]);
        header("location: ../admin_page/students.php");
    }
